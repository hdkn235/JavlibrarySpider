...
def get_video_urls(url):
    try:
        ...
        for videoTag in videoTags:
            ...
            if not videoNum in videoDic and not exist_video_db(videoNum):
                videoDic[videoNum] = href
        # 使用全局变量才能赋值
        global currentPage
...
# 添加video信息
def get_video_infos():
    if videoDic.__len__() == 0:
        print("new page url can not be found")
        return None

    try:
        for url in videoDic.values():
            # 将html转换成Python对象
            bsObj = get_pagebs(url)
            video = {}
...
def insert_video_db(video):
    if not exist_video_db(video["videoNum"]):
...
def download_imgs():
    ...
    if videoInfoList.count() == 0:
        print("new img url can not be found")
        return None
